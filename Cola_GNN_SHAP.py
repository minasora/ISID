...
ap.add_argument('--model', default='Cola-GNN', choices=['cola_gnn','CNNRNN_Res','RNN','AR','ARMA','VAR','GAR','SelfAttnRNN','lstnet','stgcn','dcrnn'], help='')
...
import shap
background = X[:-1]
test_images = X[-1:] #单样本 输出格式 [1, 47] 输入格式[1, 20, 47, 1]

e = shap.DeepExplainer(model, background)
shap_values = e.shap_values(test_images)

shap_numpy = [np.swapaxes(np.expand_dims(s, -1), 1, 2) for s in shap_values]
test_numpy = np.swapaxes(test_images.unsqueeze(dim=-1).cpu().detach().numpy(), 1, 2)
...
prediction_result = model(test_images).cpu().detach().numpy().reshape(1,X.shape[-1])
shap.image_plot(shap_numpy[:3], pixel_values = -test_numpy, labels=prediction_result[:,:3])
